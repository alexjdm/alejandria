...
+(BOOL) insert:(User_Setup_DTO*) usuario{
    ...
    BOOL success =  [db executeUpdate:@"INSERT INTO user_setup (idUsuario, correo_electronico, nombres, apellidos, ultimo_login, password, telefono, genero, direccion) VALUES (?,?,?,?,?,?,?,?,?);",usuario.idUsuario, usuario.correo_electronico, usuario.nombres, usuario.apellidos, usuario.ultimo_login, usuario.password, usuario.telefono, usuario.genero, usuario.direccion, nil];
...
+(User_Setup_DTO*) getUser:(NSNumber*) idUsuario {
    ...
    FMResultSet *results = [db executeQuery:@"SELECT * FROM user_setup where idUsuario=?", idUsuario];
    int resultsCount = 0;
    User_Setup_DTO *user = [[User_Setup_DTO alloc] init];
    int iduser;
    while([results next])
